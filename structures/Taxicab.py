    ...
    def learn(self, rounds, test_interval=50, test_rounds=250):
        ...
        for r in range(1, rounds + 1):
            ...
            while self.S.time_period <= self.instance.last_period:
                # check if this is the last period BEFORE transition
                is_last_period = True if self.S.time_period == self.instance.last_period else False
                ...
                # feed forward (learn)
                # OLD: self.policy.feed_forward(last_period=True if self.S.time_period == self.instance.last_period else False)
                self.policy.feed_forward(last_period=is_last_period)
            ...
            if r % 1000 == 0:
                print('Learn', self.policy_name, 'in round', r, 'of', rounds, ', avg. reward=',
                      round(statistics.mean(self.test(rounds=1000)), 4), time.strftime("%H:%M:%S", time.localtime()),
                      f" ({int((t := time.time() - computation_time_start) // 60)} min {int(t % 60)} sec)")
